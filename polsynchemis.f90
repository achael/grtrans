...
!          write(6,*) 'x: '!,xmin,xmax,real(p),gmin,gmax
          call get_synchpl_facs(xmin,xmax,p,gxfac, gafac)
...
!                  write(6,*) 'synchpl facs',ix1,iy1
      ...
      subroutine polsynchth(nu,n,b,t,theta,fpositron,e)
          ...
          integer :: testindx
!         write(*,*) 'polsynchth vars: ',size(B),size(T),size(n),size(nu)
...
!          (besselk0(1d0/thetae)-(sign(1d0,thetae+1d0)-1d0)/2d0*shgmfunc(xarg))/besselk(2,1d0/thetae)*cos(theta)
          ...
          where(thetae.gt.1d-2)
             eps11m22=jffunc(xarg)*wp2*omega0**2/(2d0*pi*nu)**4* &
                  (besselk1(1d0/thetae)/besselk(2,1d0/thetae)+6d0*thetae)*sin(theta)**2
             eps12=wp2*omega0/(2d0*pi*nu)**3* &
                  (besselk0(1d0/thetae)-(sign(1d0,thetae+1d0)-1d0)/2d0*shgmfunc(xarg))/besselk(2,1d0/thetae)*cos(theta)
          elsewhere
             eps11m22=jffunc(xarg)*wp2*omega0**2/(2d0*pi*nu)**4* &
...
!          tm=-(eps11m22+targ)/2d0/eps12
          ...
          ! confusion about sign of rhoq. Shcherbakov (2008) disagrees with Huang & Shcherbakov (2011).
          ! Statement is different basis vectors. Basis looks identical between SH10, which I follow, and HS11. 
          ! So I did signs as in Huang & Shcherbakov.
